...
/**
 * These are unit tests for a [WifiDirectServiceConnectionEngine]
...
class UnitWifiDirectServiceConnectionEngine {
    ...
    fun setup() {
        ...
        //Run
        WifiDirectServiceConnectionEngine.getInstance()
    ...
    @After
    fun teardown() {
        WifiDirectServiceConnectionEngine.getInstance().teardownEngine()
    }
    ...
    fun itShouldRegisterServiceForDiscovery() {
        ...
        val result = WifiDirectServiceConnectionEngine.getInstance().registerService(description, peer)
    ...
    fun itShouldNotRegisterASecondService() {
        val peer = TestPeer()
        val description = testDescriptionOne
        WifiDirectServiceConnectionEngine.getInstance().registerService(description, peer)
        val result = WifiDirectServiceConnectionEngine.getInstance().registerService(description, peer)
        verify(exactly = 1) { mockedDiscovery.startService(description) }
        verify(exactly = 1) { mockedDiscovery.startDiscoveryForService(description) }
        assertFalse(result)
    ...
    fun itShouldNotifyAboutBecomingGroupOwner() {
        ...
        WifiDirectServiceConnectionEngine.getInstance().onBecameGroupOwner()
    ...
    fun itShouldNotifyAboutBecomingAClient() {
        ...
        WifiDirectServiceConnectionEngine.getInstance().onBecameClient()
    ...
    fun itShouldNotifyWhenAConnectionHasBeenCreated() {
        ...
        WifiDirectServiceConnectionEngine.getInstance().onSocketConnected(connection);
